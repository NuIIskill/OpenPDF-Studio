...
public:
        ...
        , m_index(index), m_first(first), m_last(last)
    {
        ...
        m_marker->setFixedSize(kMarkerSize, kMarkerSize);

        auto *markerCol = new QWidget(this);
        markerCol->setFixedWidth(kMarkerCol);
...
protected:
    ...
    void paintEvent(QPaintEvent *e) override
    {
        ...
        const int cx  = kMarkerCol / 2;
        const int top = height() / 2 - kMarkerSize / 2;
        const int bot = height() / 2 + kMarkerSize / 2;
